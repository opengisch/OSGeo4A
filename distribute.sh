...
function push_arm() {
  ...
  if [ "X${ANDROIDNDKVER:0:2}" == "Xr7" ] || [ "X${ANDROIDNDKVER:0:2}" == "Xr8" ]; then
    ...
    export TOOLCHAIN_DIR=${TOOLCHAIN_BASEDIR}-${TOOLCHAIN_VERSION}
    export TOOLCHAIN_CC=${TOOLCHAIN_PREFIX}-gcc
    export TOOLCHAIN_CXX=${TOOLCHAIN_PREFIX}-g++
  ...
  if [ "X${TOOLCHAIN_CC}" == "Xclang" ]; then
    CFLAGS+=" -target thumbv7-none-linux-androideabi -gcc-toolchain ${ANDROIDNDK}/toolchains/${TOOLCHAIN_BASEDIR}-${TOOLCHAIN_VERSION}/prebuilt/${PYPLATFORM}-x86_64"
  else
    CFLAGS+=" -mandroid"
    if [ "X$ARCH" == "Xarmeabi-v7a" ]; then
      CFLAGS+=" -march=armv7a -mfloat-abi=softfp -mfpu=vfp -mthumb"
    fi
  fi
  ...
  export PATH="$STAGE_PATH/bin:$ANDROIDNDK/toolchains/${TOOLCHAIN_DIR}/prebuilt/$PYPLATFORM-x86/bin/:$ANDROIDNDK/toolchains/${TOOLCHAIN_DIR}/prebuilt/$PYPLATFORM-x86_64/bin/:$ANDROIDNDK/toolchains/${TOOLCHAIN_BASEDIR}-${TOOLCHAIN_VERSION}/prebuilt/${PYPLATFORM}-x86_64/bin:$ANDROIDNDK:$ANDROIDSDK/tools:$QTSDK/android_$QT_ARCH_PREFIX/bin:$PATH"

  # search compiler in the path, to fail now instead of later.
  CC=$(which ${TOOLCHAIN_CC})
  if [ "X$CC" == "X" ]; then
    error "Unable to find compiler (${TOOLCHAIN_CC}) !!"
  ...
  export CC="${TOOLCHAIN_CC} $CFLAGS"
  export CXX="${TOOLCHAIN_CXX} $CXXFLAGS"
